...
public class AddJarDialog extends JDialog implements ActionListener
{
    ...
    public AddJarDialog(GeneratorUI gui)
    {
        motherComp = gui;
        setModal(true);
    ...
    private void init()
    {
        ...
        ArrayList jarList = motherComp.getExternalJars(); 
        for(int i = 0; i < jarList.size(); i ++)
        {
            String actualLib = ((File)jarList.get(i)).getAbsolutePath();
            File fileJar    = new File(actualLib);
            if(fileJar.canRead())
            {
                //mit gutem symbol
                listmodel.addElement(((File)jarList.get(i)).getAbsolutePath());
            }
            else
            {
                //mit schlechtem Symbol
            }
        }
        ...
        but_add     = new JButton("Add Jar archive");
        but_remove  = new JButton("Remove selected Jars");
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {
            setTitle("Please wait a sec while your jars are parsed");
            motherComp.getJarFiles(false, motherComp.getExternalJars());
            dispose();
        }
        if(e.getSource() == but_remove)
        {
            ...
            {
                motherComp.removeExternalJar(selection[i-1]);
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION)
            {
                ...
                for(int i = 0; i < files.length; i++)
                {
                    if(files[i].isFile() && files[i].canRead() && files[i].getName().endsWith(".jar"))
                    {
                        ...
                        //TODO mit gutem Symbol
                        listmodel.addElement(strFilePathandName);
                        motherComp.addExternalJar(files[i]);
                    }
                }
            }
            if(listmodel.size() > 0)
