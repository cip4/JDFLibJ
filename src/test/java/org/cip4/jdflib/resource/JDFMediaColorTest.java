...
class JDFMediaColorTest
{
	...
	void testMediaColor()
	{
		...
		final String strNode = "<MediaIntent xmlns=\"http://www.CIP4.org/JDFSchema_1_1\" ID=\"MI100000\" Class=\"Intent\" Status=\"Available\">"
				...
				+ "<Grade      DataType=\"IntegerSpan\" Preferred=\"1\" Actual=\"1\"/>" + "</MediaIntent>";

		final JDFDoc jdfDoc = p.parseString(strNode);

		// no class cast exception on the following two lines means ok ...
		final JDFMediaIntent mediaIntent = (JDFMediaIntent) jdfDoc.getRoot();
		...
		final JDFSpanNamedColor nc = mediaIntent.getMediaColor();
		Assertions.assertEquals(nc.getPreferred().name(), "White");
	}
}
