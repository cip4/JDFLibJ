...
public class PrintfFormatTest extends JDFTestCaseBase {
	...
	/**
	 * Tests the class PrintfFormat. If everything is OK, the string "Passed" is printed, and the program exits with status 0. Otherwise, diagnostics and a
	 * stack trace are printed, and the program exits with status 1.
	 * 
	 * @param args Program arguments.<br>
	 * "-help" prints a usage message.<br>
	 * "-timing" compares how long it takes to output a double compared to the regular Java string conversion.
	 */
	static public void main(String[] args) {
		...
		if (testTiming) {
			try {
				...
				System.out.println("Typical time for Double.toString(): " + texec + " usec");
				...
				t1 = System.currentTimeMillis();
				texec = (1000 * (t1 - t0)) / (double) (niter * nsamp);
				System.out.println("Typical time for PrintfFormat.tostr(double): " + texec + " usec");
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(1);
			}
	...
	static void test(String s, char type, String pfx, String sfx, int w, int p, boolean alt, boolean zeropad, boolean left, boolean blank, boolean sign) {
