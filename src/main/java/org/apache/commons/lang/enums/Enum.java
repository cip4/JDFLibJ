/*
 * 
 * this is a direct copy of the original commons.lang (not lang3) Enum class and retained here to keep source compatibility without requiring commons.lang which has known security issues and is no longer maintained
 * 
 * 
 ...
 *
 * <p>
 * One feature of the C programming language lacking in Java is enumerations. The C implementation based on ints was poor and open to abuse. The original Java recommendation and
 * most of the JDK also uses int constants. It has been recognised however that a more robust type-safe class-based solution can be designed. This class follows the basic Java
 * type-safe enumeration pattern.
 * </p>
 *
 * <p>
 * <em>NOTE:</em> Due to the way in which Java ClassLoaders work, comparing Enum objects should always be done using <code>equals()</code>, not <code>==</code>. The equals() method
 * will try == first so in most cases the effect is the same.
 * </p>
 * 
 * <p>
 * Of course, if you actually want (or don't mind) Enums in different class loaders being non-equal, then you can use <code>==</code>.
 * </p>
 * 
 * <h4>Simple Enums</h4>
 *
 ...
 * <p>
 * A hierarchy of Enum classes can be built. In this case, the superclass is unaffected by the addition of subclasses (as per normal Java). The subclasses may add additional Enum
 * constants <em>of the type of the superclass</em>. The query methods on the subclass will return all of the Enum constants from the superclass and subclass.
 * </p>
 *
 * <pre>
 ...
 *
 * <p>
 * This example will return RED, GREEN, BLUE, YELLOW from the List and iterator methods in that order. The RED, GREEN and BLUE instances will be the same (==) as those from the
 * superclass ColorEnum. Note that YELLOW is declared as a ColorEnum and not an ExtraColorEnum.
 * </p>
 * 
 ...
 *
 * <p>
 * Care must be taken with class loading when defining a static nested class for enums. The static nested class can be loaded without the surrounding outer class being loaded. This
 * can result in an empty list/map/iterator being returned. One solution is to define a static block that references the outer class where the constants are defined. For example:
 * </p>
 *
 * <pre>
 ...
 * </p>
 * <ul>
 * <li>The standard Enum is a not just a superclass, but is a type baked into the language.</li>
 * <li>The standard Enum does not support extension, so the standard methods that are provided in the Lang enum are not available.</li>
 * <li>Lang mandates a String name, whereas the standard Enum uses the class name as its name. getName() changes to name().</li>
 * </ul>
 *
 * <p>
 * Generally people should use the standard Enum. Migrating from the Lang enum to the standard Enum is not as easy as it might be due to the lack of class inheritence in standard
 * Enums. This means that it's not possible to provide a 'super-enum' which could provide the same utility methods that the Lang enum does. The following utility class is a Java
 * 5.0 version of our EnumUtils class and provides those utility methods.
 * </p>
 *
 * <pre>
 * import java.util.*;
...
public abstract class Enum implements Comparable, Serializable
{
	 ...
	 * @throws IllegalArgumentException if the getEnumClass() method returns a null or invalid Class
	 */
	...
	private void init(final String name)
	{
		...
		final Class enumClass = getEnumClass();
		if (enumClass == null)
		{
		...
		{ // LANG-334
			...
			if (entry == null)
			{
				...
				final Map myMap = new WeakHashMap(); // we avoid the (Map) constructor to achieve JDK 1.2 support
	...
	{
		final Entry entry = (Entry) cEnumClasses.get(getEnumClass());
		if (entry == null)
		{
			return null;
		}
	 ...
	 * </p>
	 * 
	 * @param enumClass the class of the Enum to get, must not be <code>null</code>
	 ...
	 *
	 * <p>
	 * The list is in the order that the objects were created (source code order). If the requested class has no enum objects an empty <code>List</code> is returned.
	 * </p>
	 * 
	 * @param enumClass the class of the <code>Enum</code> to get, must not be <code>null</code>
	 * @return the enum object Map
	 ...
	 *
	 * <p>
	 * The <code>Iterator</code> is in the order that the objects were created (source code order). If the requested class has no enum objects an empty <code>Iterator</code> is
	 * returned.
	 * </p>
	 * 
	 * @param enumClass the class of the <code>Enum</code> to get, must not be <code>null</code>
	...
	private static Entry createEntry(final Class enumClass)
	{
		...
		while (cls != null && cls != Enum.class && cls != ValuedEnum.class)
		{
			final Entry loopEntry = (Entry) cEnumClasses.get(cls);
			if (loopEntry != null)
			{
	 ...
	 * 
	 * <p>
	 * This is normally the same as <code>getClass()</code>, but for advanced Enums may be different. If overridden, it must return a constant value.
	 * </p>
	 * 
	...
	private String getNameInOtherClassLoader(final Object other)
	{
		try
		{
			final Method mth = other.getClass().getMethod("getName", null);
			final String name = (String) mth.invoke(other, null);
			return name;
		}
