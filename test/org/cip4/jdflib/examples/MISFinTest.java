...
public class MISFinTest extends JDFTestCaseBase
{
	...
	public void testAmount()
	{
		...
		JDFComponent comp = (JDFComponent) n.addResource(ElementName.COMPONENT,
				null, EnumUsage.Output, null, null, null, null);
		JDFResourceLink rl = n.getLink(comp, null);
		rl.setAmount(2, null);
		rl
				.setDescriptiveName("The link point to 2 pallets with a total comtent of 10000 brochures in 200 boxes");

		// create a pallet partition - may also use the root
		JDFComponent compPallet = (JDFComponent) comp.addPartition(
				EnumPartIDKey.DeliveryUnit0, "Pallet");

		JDFComponent compBox = (JDFComponent) compPallet.addPartition(
				EnumPartIDKey.DeliveryUnit1, "Box");
		JDFBundle bundlePallet = compPallet.appendBundle();
		bundlePallet
				.setDescriptiveName("Bundle describing 100 boxes with 5000 Brochures Contents total");
		...
		JDFComponent compBrochure = (JDFComponent) compBox.addPartition(
				EnumPartIDKey.DeliveryUnit2, "Brochure");
		JDFBundle bundleBox = compBox.appendBundle();
		bundleBox
				.setDescriptiveName("Bundle describing 1 boxes with 50 Brochures Contents per box");
		...
		bundleBrochure
				.setDescriptiveName("Dummy Bundle to inhibit inheritence of the box Bundle");
		d.write2File(sm_dirTestDataTemp + File.separator + "MISFinAmount.jdf",
				2, false);
	}
	...
	public void testAmountPalletteManifest()
	{
		...
		rl
				.setDescriptiveName("The link point to 2 pallets with a total comtent of 10000 brochures in 200 boxes");
		JDFComponent compBrochure = (JDFComponent) n.addResource(
				ElementName.COMPONENT, null, EnumUsage.Input, null, null, null,
				null);
		...
		for (int i = 0; i < 2; i++)
		{
			// create a pallet partition - may also use the root
			JDFComponent compPallet = (JDFComponent) comp.addPartition(
					EnumPartIDKey.DeliveryUnit0, "Pallet" + i);
			compPallet.setProductID("Pallet_" + i);

			JDFBundle bundlePallet = compPallet.getCreateBundle();
			int nBox = i == 0 ? 100 : 94;
			bundlePallet.setDescriptiveName("Pallet Bundle describing " + nBox
					+ " boxes with " + nBox * 50 + " Brochures Contents total");
		...
		d.write2File(sm_dirTestDataTemp + File.separator
				+ "MISFinAmountManifest.jdf", 2, false);
	}
	...
	public void testAmountPalletteCompleteManifest()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			for (int j = 0; j < nBox; j++)
			{
				JDFBundleItem biBoxes = bundlePallet.appendBundleItem();
				JDFComponent compBox = (JDFComponent) compPallet.addPartition(
						EnumPartIDKey.DeliveryUnit1, "Box_" + i + "_" + j);
				compBox.setProductID("Box_" + i + "_" + j);
				biBoxes.refElement(compBox);
				biBoxes.setAmount(1);

				JDFBundle bundleBox = compBox.appendBundle();
				bundleBox.setDescriptiveName("Bundle describing box #" + j
						+ " with 50 Brochures Contents per box");
				bundleBox.setTotalAmount(50);
				bundleBox.setBundleType(EnumBundleType.Box);
		...
		d.write2File(sm_dirTestDataTemp + File.separator
				+ "MISFinAmountCompleteManifest.jdf", 2, false);
	}
