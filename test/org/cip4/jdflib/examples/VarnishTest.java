...
public class VarnishTest extends JDFTestCaseBase
{
	...
	public void testCombinedVarnish() throws Exception
	{
		...
		JDFConventionalPrintingParams cpp = (JDFConventionalPrintingParams) n
		...
		cpp
				.appendXMLComment(
						"Module 0 and 7 are varnishing modules, 1-4 are process colors and 6 is the ink module used to varnish",
						null);
		n.appendMatchingResource("Component",
		...
		JDFInk ink = (JDFInk) n.appendMatchingResource("Ink",
				JDFNode.EnumProcessUsage.AnyInput, null);

		JDFResource vp = n.addResource("VarnishingParams",
				EnumResourceClass.Parameter, EnumUsage.Input, null, null, null,
				null);
		JDFExposedMedia xmVarnish = (JDFExposedMedia) n.addResource(
		...
		JDFColorantControl cc = (JDFColorantControl) n.appendMatchingResource(
				ElementName.COLORANTCONTROL, JDFNode.EnumProcessUsage.AnyInput,
				null);
		cc
				.getCreateDeviceColorantOrder()
				.appendXMLComment(
						"Should the VarnishingParams seps be excluded, as is shown here?",
						null);
		...
		for (int i = 0; i < vSeparations.size(); i++)
		{
			...
			if (sep.equals("Varnish2"))
			{
				vp
						.appendXMLComment(
								"full varnishing in a varnishing module with or wihtout a sleeve. Full varnishing means to cover the complete media surface.",
								null);
				xmVarnish.addPartition(EnumPartIDKey.Separation, sep);
				JDFResource varnishPart = vp.addPartition(
						EnumPartIDKey.Separation, sep);
			...
			} else if (sep.equals("PreVarnish"))
			{
				vp
						.appendXMLComment(
								"varnishing in a varnishing module only with a mandatory prepared sleeve ",
								null);
				xmVarnish.addPartition(EnumPartIDKey.Separation, sep);
				JDFResource varnishPart = vp.addPartition(
						EnumPartIDKey.Separation, sep);
